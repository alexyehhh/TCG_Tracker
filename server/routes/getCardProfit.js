...
router.get('/card-profit', async (req, res) => {
    ...
    if (expeditedTurnaround && salePrice > 500) {
        return res.status(400).json({ error: 'Expedited Turnaround is only available for cards with a sale price of $500 or less.' });
    }
    ...
    try {
        ...
        if (salePrice <= 500) {
            ...
            psaGradingCost = 129.00;
        } else if (salePrice <= 5000) {
            psaGradingCost = 249.00;
        } else if (salePrice <= 10000) {
            psaGradingCost = 499.00;
        } else {
